...
def derive(events, overrides=None):
    ...
    for i, e in enumerate(events):
        ...
        row = {
            ...
            # single recognition item (turn 11) is presented via forced_choice_block;
            # recognition_items is retained for audit but only item 0 is shown.
            ...
            # #1/#4: the runner presents ONE conversational recognition item, not a barrage.
            # A hand-authored `recognition_stem` in the override (distinct distractor,
            # referent-bearing) wins; otherwise the override forced_choice is used.
...
def main():
    ...
    out = {
        ...
        "derivation_rules": {
            ...
            "recognition_items": "audit-only multi-item set (NOT presented). Turn 11 presents the single forced_choice_stem via forced_choice_block; see recognition_policy and distractor_uniqueness.",
        },
        "override_policy": {
        ...
        "recognition_policy": {
            "presentation": "SINGLE conversational forced-choice item at turn 11 (smoke-run fix #1/#4) - the multi-item barrage was softened. forced_choice_block is the one stem the runner fills into {forced_choice_stem}. recognition_items is retained for audit; only item 0 is presented.",
            "scoring": "binary correct/incorrect on the one item.",
        },
